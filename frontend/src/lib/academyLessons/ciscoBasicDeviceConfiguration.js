...
function buildQuiz() {
  return [
    ...
    { question: 'Ein Interface ist versehentlich mit "shutdown" deaktiviert worden. Wie hebst du das auf?', options: ['no shutdown', 'enable shutdown', 'undo shutdown', 'default shutdown'], correct: 0, explanation: '"no shutdown" deaktiviert den "shutdown"-Befehl und aktiviert das Interface wieder.' },
    { question: 'Du hast "hostname SW-Wrong" eingegeben und merkst, dass der Name falsch sein soll. Wie korrigierst du ihn?', options: ['no hostname', 'hostname SW-Right', 'reload', 'write erase'], correct: 1, explanation: 'Indem du den Befehl mit dem korrekten Wert einfach erneut eingibst, überschreibst du die vorherige Einstellung - das ist die normale Art, Konfigurationswerte in IOS zu korrigieren.' },
    { question: 'Wie prüfst du aus dem Interface-Konfigurationsmodus, ob der Hostname korrekt gesetzt wurde, ohne den Modus zu verlassen?', options: ['show running-config', 'do show running-config', 'display hostname', 'verify hostname'], correct: 1, explanation: '"do show running-config" führt den Privileged-EXEC-Befehl direkt aus dem Konfigurationsmodus aus und zeigt die gesamte Konfiguration inklusive Hostname.' },
    { question: 'Du hast einen falschen Befehl eingegeben, der die Konfiguration verändert hat. Welches Werkzeug nutzt du zuerst, um das rückgängig zu machen?', options: ['reload', '"no" oder den korrigierten Befehl erneut eingeben', 'write memory', 'erase startup-config'], correct: 1, explanation: 'IOS hat kein generelles Undo. Stattdessen verwendet man "no <Befehl>" oder gibt den korrekten Wert erneut ein, um die Konfiguration anzupassen.' },
  ];
}
