// Phase 1J: Verify procedural generator unlocks after HM2 and HM3.
...
const { MISSION_001_ID, MISSION_002_ID, MISSION_003_ID } = await import(pathToFileURL(join(srcDir, 'lib/missionV2.js')).href);
...
console.log('Template unlock logic');
{
  ...
  for (const id of stage3Templates) {
    test(`${id} locked before HM3`, () => assert(!isTemplateUnlocked(getTemplate(id))));
  }

  makeState([MISSION_001_ID, MISSION_002_ID, MISSION_003_ID]);
  for (const id of stage3Templates) {
    test(`${id} unlocked after HM3`, () => assert(isTemplateUnlocked(getTemplate(id))));
  }
}
...
console.log('\nGeneratable skill paths');
{
  ...
  test('routing paths after HM3', () => assert(pathsAfter3.some((p) => p.startsWith('cisco.routing'))));
}
...
console.log('\nGenerated instances respect unlocks');
{
  ...
  test('stage-3 instances can generate after HM3', () => assert(foundStage3));
}
